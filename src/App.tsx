...
import { Song } from './Components/Data/data';
...
function App() {
  ...
  function handleSubmit(song: Song) {
    const savedPlaylist = userPlayList.map(song => song.uri);
  }
  ...
  async function handleSearch () {
    ...
    try {
      const response = await fetch(`https://api.spotify.com/v1/search?type=track&q=${query}&limit=10`,{
        headers: {
      ...
      setSongsList(formattedTracks)
      // setSongsList(tracks.map((track) => ({
      //   id: track.id,
      //   name: track.name,
      //   artists: track.artists[0].name, 
      //   uri: track.uri,
  ...
  return (
    <div className={styles.main}>
      ...
      <div className={styles.lay}>
        ...
        <PlayList name={playListName} onChange={handleNameChange}>
